...
var handler = async (m, { conn, text }) => {
  ...
  try {
    // Llama a la API de ipinfo para obtener la información de la IP
    let response = await fetch(`https://ipinfo.io/${ip}?token=1f3c8717fa72a9`);
    let data = await response.json();
    
    let doxeo = `*_✅ Información de la IP obtenida con éxito_*\n\n*IP:* ${ip}\n*Ciudad:* ${data.city}\n*Región:* ${data.region}\n*País:* ${data.country}\n*Organización:* ${data.org}\n*Ubicación:* ${data.loc}\n*Zona horaria:* ${data.timezone}\n*Código postal:* ${data.postal}`;

    await conn.sendMessage(m.chat, { text: doxeo }, { quoted: m });
  } catch (error) {
    await conn.sendMessage(m.chat, { text: `❌ Error al obtener información de la IP: ${error.message}` }, { quoted: m });
  }
...
handler.help = ['ipinfo'];
handler.tags = ['tools'];
handler.command = /^ipinfo|iplookup|ipbuscar$/i;
handler.group = true;
