...
if (new Date - user.lastmiming < 10000) return await conn.reply(m.chat, `*ESPERA UNOS MINUTOS PARA USAR OTRO COMANDO*`,  m)
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      if (!img) throw `𝙼𝙴𝙶𝚄𝙼𝚄𝙽 𝙱𝙾𝚃 𝚃𝙴 𝙰𝚈𝚄𝙳𝙰 

𝙿𝙰𝚁𝙰 𝙲𝚁𝙴𝙰𝚁 𝚃𝚄 𝚂𝚃𝙸𝙲𝙺𝙴𝚁 𝚁𝙴𝚂𝙿𝙾𝙽𝙳𝙴 𝙰 𝚄𝙽𝙰 𝙸𝙼𝙰𝙶𝙴𝙽 𝙾 𝚅𝙸𝙳𝙴𝙾 𝙲𝙾𝙽 𝙴𝙻 𝙲𝙾𝙼𝙰𝙽𝙳𝙾   .s *${usedPrefix + command}_*`
      let out
      try {
...
await conn.reply(m.chat, `${eg}⏳ *PROCESANDO SU STICKER, POR FAVOR UN MOMENTO...* `, m)
    ...
    else throw '𝙿𝙰𝚁𝙰 𝙲𝚁𝙴𝙰𝚁 𝚃𝚄 𝚂𝚃𝙸𝙲𝙺𝙴𝚁 𝚁𝙴𝚂𝙿𝙾𝙽𝙳𝙴 𝙰 𝚄𝙽𝙰 𝙸𝙼𝙰𝙶𝙴𝙽 𝙾 𝚅𝙸𝙳𝙴𝙾 𝙲𝙾𝙽 𝙴𝙻 𝙲𝙾𝙼𝙰𝙽𝙳𝙾   .s'
  }
